...
- **港美股**：新浪财经主报价 + 腾讯个股字段补充，东财 `stock/get` 精确兜底，clist 批量补充，富途与新浪资讯多源 fallback
...
- `young-stock-cli>=0.1.19`（核心行情、缓存、交易日、基金、日报编排和本地管理命令）
- `curl` (所有系统通用)
...
| data.10jqka.com.cn | **A股资金流** | 概念板块流入、流出、净额 | **不需要** | 仅在净流入和净流出榜均可用时作为资金流方向源 |
...
东财 clist 适合做港美股批量补充，但它按榜单返回，港股 `m:128` 可能优先返回牛熊证/衍生品，美股 `m:105,m:106,m:107` 也可能优先返回小盘或权证，导致腾讯、阿里、苹果、英伟达等重点正股不在本地过滤结果中。v3.6.5 起，港股指数优先用腾讯财经收盘口径，港美重点个股优先调用新浪财经批量行情；腾讯个股行情用于补充成交额、换手率、市值、PE/PB、52周区间，缺失价格时也作为东财 `stock/get` 前的兜底，clist 仅作批量补充。

| 维度 | 腾讯财经 | 新浪财经 | 东财 stock/get / clist |
|---|---|---|---|
| 登录/API Key | 不需要 | 不需要 | 不需要 |
| 使用时机 | 港股指数优先；A/HK/US 个股补充成交额和估值字段 | 港美个股主报价；指数补充 | 新浪/腾讯缺失时兜底/补充 |
| 批量获取 | ✅ | ✅ | `stock/get` 逐只；clist 批量 |
| 美股指数 | ✅ SPX、NDX、DJI | ✅ SPX、NDX、DJI | ✅ SPX、NDX、DJI |
| 美股重点股 | ✅ 补成交额、换手率、市值、PE、52周区间 | ✅ AAPL、NVDA、TSLA、MSFT、AMZN、GOOGL、META、BABA、PDD、JD | ✅ 常见重点股 |
| 港股指数 | ✅ HSI、HSCEI、HSTECH | ✅ HSI、HSCEI、HSTECH | ✅ HSI、HSCEI、HSTECH |
| 港股重点股 | ✅ 补成交额、市值、PE/PB、52周区间 | ✅ 0700、9988、3690、9618、1299、2318、0005、0388 | ✅ 常见重点股 |
...
- **港美股行情**：港股指数优先腾讯收盘口径；港美重点个股优先新浪财经主报价，并用腾讯个股补充成交额、市值、PE/PB、52周区间；缺失时再用东财 `stock/get` 兜底，clist 作为补充。东财 `fltt=2` 当前返回真实价，不再除以 100。
...
- **同花顺资金流**：`funds/gnzjl` 只作为概念板块资金流方向参考，不替代指数或个股行情主路径；若触发风控、不可用或只返回单边数据，会继续降级到东财、新浪、腾讯和本地缓存。
...
### v3.6.5
- 同步 young-stock-cli 0.1.19：基于 simonlin1212/global-stock-data 与 a-stock-data 的腾讯 `qt.gtimg.cn` 字段评估，新增 A股/港股/美股个股腾讯行情补充源。
- 单票、港美重点个股和基金持仓股行情会在主报价之外补充成交额、换手率、市值、PE/PB、52周高低等字段；价格主来源仍按新浪/腾讯/东财稳定链路自动选择。
- 实测样本：`600519`、`0700.HK`、`AAPL` 均能补齐此前未显示的估值/流动性字段；A股 52周字段与港美字段位置不同，已单独校准。
