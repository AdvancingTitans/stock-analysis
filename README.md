...
- **港美股**：新浪财经批量行情为主，东财 `stock/get` 精确兜底，clist 批量补充，富途免登录资讯搜索
...
东财 clist 适合做港美股批量补充，但它按榜单返回，港股 `m:128` 可能优先返回牛熊证/衍生品，美股 `m:105,m:106,m:107` 也可能优先返回小盘或权证，导致腾讯、阿里、苹果、英伟达等重点正股不在本地过滤结果中。v3.2.0 起，脚本优先调用新浪财经批量行情；新浪缺失时再用东财 `stock/get` 单只精确兜底，clist 作为批量补充。

| 维度 | 新浪财经 | 东财 stock/get / clist |
|---|---|---|
| 登录/API Key | 不需要 | 不需要 |
| 使用时机 | 首选 | 新浪缺失时兜底/补充 |
| 批量获取 | ✅ | `stock/get` 逐只；clist 批量 |
| 美股指数 | ✅ SPX、NDX、DJI | ✅ SPX、NDX、DJI |
| 美股重点股 | ✅ AAPL、NVDA、TSLA、MSFT、AMZN、GOOGL、META、BABA、PDD、JD | ✅ 常见重点股 |
| 港股指数 | ✅ HSI、HSCEI、HSTECH | ✅ HSI、HSCEI、HSTECH |
...
| **美股** | ✅ 新浪/东财 (SPX、NDX、DJI) | ✅ 新浪/东财 | ✅ 富途（代码） | ✅ 富途 |
| **港股** | ✅ 新浪/东财 (HSI、HSCEI、HSTECH) | ✅ 新浪/东财 | ✅ 富途（代码） | ✅ 富途 |
| **日股** | ⚠️ 仅 Exa | ⚠️ 仅 Exa | ⚠️ 仅 Exa | ⚠️ 仅 Exa |

## 注意事项

- **A股**：东财 API 非官方公开接口，字段名可能调整。涨跌停池 `date` 参数只返回**当日**数据。
- **港美股行情**：优先新浪财经；缺失时用东财 `stock/get` 兜底，clist 作为补充。东财 `fltt=2` 当前返回真实价，不再除以 100。
