...
当前 CLI 版本为 `4.3.0`；Skill 规则版本为 `4.3.2`。
...
持仓绩效分析遵循投资记忆优先：默认读取 `~/.young_stock/profile.json` 或 `YOUNG_STOCK_PROFILE`。如果本地 young profile 投资记忆完整，下次用户询问市场或持仓时默认按这份投资记忆分析持仓，除非用户明确已经清空过投资记忆。

投资记忆不存在且用户主动提供持仓信息，或投资记忆有但信息不完整时，进入一次确认流程。只有股票代码、买入日期、买入数量或买入金额三项完整，才输出收益、浮盈亏、年化表现和组合绩效；缺少任意一项时，默认生成普通市场复盘报告，不包含持仓绩效内容，并只提问一次精准提示补齐缺失项。
