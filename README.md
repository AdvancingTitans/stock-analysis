...
- 用户主动触发且信息完整时的股票/基金持仓、汇率折算、浮盈亏、集中度、重复暴露和基准比较
...
持仓绩效分析遵循用户主动触发规则：只有用户主动要求分析持仓，并同时提供股票代码、买入日期、买入数量或买入金额，才输出收益、浮盈亏、年化表现和组合绩效。缺少任意一项时，默认生成普通市场复盘报告，不包含持仓绩效内容；若用户本轮主动提到持仓，可在末尾只提问一次，精准提示补齐缺失项。

如果买入日期没有年份，按当前年份计算；如果用户只给出数字，必须确认这是买入数量还是买入金额，并确认币种是人民币、港币还是美元。已保存的 young profile 只能作为用户主动触发后的辅助来源，不能自动把普通复盘升级为持仓分析。
