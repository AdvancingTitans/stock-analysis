...
2. **港股代码格式**：可用 `0700`（去掉前导 0）或 `00700`。Yahoo Finance 必须用 `0700.HK`。
3. **社区数据量不稳定**：小市值标的社区讨论可能极少或空。社区情绪仅代表平台内用户，不是全市场。
4. **返回为空时的处理**：如果 `code != 0` 或 `data` 为空，不编造结果，告知用户"暂无相关数据"。
5. **英文语言匹配**：美股搜索建议用 `lang=en`，港股/A股用 `lang=zh-CN`。
