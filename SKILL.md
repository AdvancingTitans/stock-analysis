---
name: a-stock-market
description: "全球股市行情+情绪分析：A股（东财免登录API）、港美股（Yahoo Finance免登录API + 富途免登录资讯）、板块榜浏览器抓取、舆情交叉验证。v2.1.0 新增数据质量验证、指数退避重试、全球市场概览。"
version: 2.1.0
author: Hermes Agent + yjw
tags: [stock-market, a-shares, hk-shares, us-shares, eastmoney, futu, yahoo-finance, sentiment, global-finance, data-quality]
...
> ⚠️ **数据质量声明**
> - Yahoo Finance API 对不同市场的支持程度不同：美股数据最完整，港股/欧股/日股可能缺少成交量等字段
> - 异常数据（如成交量为0或明显不合理值）会被自动检测并标记，不会直接当作有效数据使用
> - 脚本内置指数退避重试机制，降低 API 限流影响
> - 数据仅供参考，不构成投资建议
...
脚本内置 `validate_and_clean_quote()` 函数，对每条数据自动检测异常值：

| 检测项 | 规则 | 处理方式 |
|---|---|---|
| 指数成交量 | < 1,000,000 视为异常 | 保留原值但标记 `volume_anomaly` |
| 个股成交量 | < 1,000 视为异常 | 保留原值但标记 `volume_anomaly` |
| 成交量为0 | `<= 0` | 标记为 `volume_zero`，显示时加 `*` 警示 |
| 价格异常 | Open/High/Low/Close/Price `<= 0` | 置为 `None`，不输出 |
| 完整性评分 | 必填字段（Price、PreviousClose、Volume） | 0-100 分，低于 80 分在报告中提示 |

### 重试机制

所有网络请求均通过 `@retry_with_backoff` 装饰器：
- 最大重试 3 次
...
| 市场 | 行情数据 | 新闻/情绪 | 板块/概念 | 降级方案 |
|---|---|---|---|---|
| **A股** | 东财 API (主) | 富途 news_search | 东财页面抓取 | Yahoo Finance (个股补充) |
| **港股** | Yahoo Finance (主) | 富途 news_search | 富途/东财页面 | 东财 API (沪深港通) |
| **美股** | Yahoo Finance (主) | 富途 news_search | Yahoo/富途页面 | 无（Yahoo 覆盖最全） |
| **日股** | Yahoo Finance (主) | Exa 搜索 | 东财国际页面 | 无 |
| **欧股** | Yahoo Finance (主) | Exa 搜索 | 浏览器抓取 | 无 |

### 市场类型自动检测

脚本通过 `detect_market_type()` 自动识别标的所属市场：

| 检测规则 | 结果 |
|---|---|
| `.HK` 后缀、`HSI`/`HSCE`/`HSTECH` | hk_market |
| `上证`、`深证`、`创业板`、`科创板`、`399001`、`899050` | cn_market |
...
- **完整 (90-100%)**: 所有必填字段均正常
- **可用 (70-89%)**: 部分字段异常或缺失，但核心数据完整
- **局部缺失 (< 70%)**: 建议检查网络或换时段重试

## Limitations

- A股数据（东财）仅支持沪深京。港/美/日股不支持涨跌停池、连板梯队等 A股专属指标。
- Yahoo Finance API 免登录但有频率限制，过快会被封 IP。脚本内置指数退避重试机制以缓解此问题。
- 富途免登录 Search Skills 主要覆盖港美股标的新闻，A股标的覆盖率因标的而异。
- 东财 API 是非官方公开接口，字段名可能调整。
- 真实交易日才有数据；周末/节假日返回前一交易日或空。
- 富途社区情绪仅反映平台内讨论，不代表全市场。
- 港股/欧股/日股的成交量数据来源（Yahoo Finance）可能不完整，部分指数成交量字段返回为0或缺失，已在脚本中做异常检测和标记。
