...
`stock-analysis` 固定负责 M1-M6 的证据包、评分和研报正文；chat、report、send、外部发布和真实交易属于宿主 Agent 或应用的上层产品工作流。需要多专家综合时，先用本 skill 产出可审计证据，再由宿主基于内置投资框架做综合判断。

## 内置投资框架

当用户明确提出想用哪位投资专家的风格生成报告时，必须识别专家名称、英文名、中文名、别名或框架 id，并完全以相关专家的视角输出报告。这里的“完全以相关专家的视角”指整篇报告的证据优先级、判断顺序、风险表达、持仓建议和观察清单都服从该专家框架，不得只在结尾追加专家点评，也不得先写 balanced 普通报告再附一段专家口吻。

支持 15 个内置投资框架：

| 框架 id | 中文名 | 框架要点 | 证据优先级 |
|---|---|---|---|
...
- 只有用户明确说“用巴菲特/芒格/彼得·林奇/索罗斯/段永平/张坤/冯柳等风格”“按 buffett/munger 框架风格”“以某投资专家视角”等，才切换专家框架。
- 用户没有明确指定专家时，保持 balanced 证据驱动复盘，不主动询问要不要选择专家。
- 识别到单个专家时，单专家视角不输出多专家综合判断，也不输出“交易计划草案”“风险管理意见”“组合经理最终意见”等委员会小节。
...
- 若用户同时指定多个专家，除非明确要求“全部”或“多专家综合”，否则只执行用户最后明确指定的一个专家；明确要求多专家综合时，可以在 evidence pack 之上输出综合视角，但不得伪造逐轮辩论。

## 投资记忆优先与持仓完整性

持仓相关请求优先读取本技能投资记忆。若用户要求分析持仓，或 CLI 显式传入 `--with-holdings`，先读取本地投资记忆并进行持仓绩效分析；普通行情复盘不得因为本机存在投资记忆而自动插入持仓绩效。
...
没有主动要求持仓分析的用户不会被打扰。若用户要求持仓分析但本地没有完整投资记忆，必须等待用户交互输入并只提问一次，要求补齐缺失项。
...
只要股票代码、买入日期、买入数量或买入金额缺失任意一项，输出普通市场复盘报告，不包含任何持仓绩效内容。若触发确认流程，可以在报告末尾只提问一次，精准说明缺失项，例如：
...
- 显式读取投资记忆 `~/.stock_analysis/profile.json` 或 `STOCK_ANALYSIS_PROFILE`。
