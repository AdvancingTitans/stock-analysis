---
name: stock-analysis
description: 全球股市深度复盘技能。用于 A股、港股、美股、基金的当前行情、盘中/盘后复盘、6 模块证据驱动分析、young profile 投资记忆优先的持仓分析、young-stock-cli 投资专家 lens、单股速览与数据源诊断；执行腾讯/新浪优先、东财独有数据限流、证据质量评分和浏览器接管策略。
metadata:
  version: "4.3.4"
...
- 投资记忆优先：默认读取 `~/.young_stock/profile.json` 或 `YOUNG_STOCK_PROFILE`；完整投资记忆可直接触发持仓分析。
...
支持 15 个 young-stock-cli 投资专家 lens：
...
- 只有用户明确说“用巴菲特/芒格/彼得·林奇/索罗斯/段永平/张坤/冯柳等风格”“按 buffett/munger/lens 风格”“以某投资专家视角”等，才切换专家 lens。
- 用户没有明确指定专家时，保持 balanced 证据驱动复盘，不主动询问要不要选择专家。
- 识别到单个专家时，单专家视角不触发 M7，不输出“## M7 机构化综合判断”，也不输出“交易计划草案”“风险管理意见”“组合经理最终意见”等委员会小节。
...
- 若用户同时指定多个专家，除非明确要求 `all` 或“多专家综合”，否则只执行用户最后明确指定的一个专家；明确要求 `all` 时交给 young-stock-cli 的 `--lens all`/M7 工作流，不在本 skill 内伪造辩论。

## 投资记忆优先与持仓完整性

持仓分析遵循投资记忆优先。若本地存在完整 young profile 投资记忆，默认读取投资记忆并进行持仓绩效分析；下次用户问市场或持仓时默认以上次记录的投资记忆进行分析，除非用户明确已经清空过投资记忆。

投资记忆路径：

- 默认：`~/.young_stock/profile.json`
- 覆盖：`YOUNG_STOCK_PROFILE`

没有主动提供持仓信息的用户不会被打扰：若本地也没有完整投资记忆，只输出普通市场复盘报告。
...
- 默认读取投资记忆 `~/.young_stock/profile.json` 或 `YOUNG_STOCK_PROFILE`。
