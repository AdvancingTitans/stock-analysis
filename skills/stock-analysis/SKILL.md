...
- `--market stock --symbol <代码>` 与 `--market fund --symbol <代码>` 是确定性速览入口，不触发 LLM；A股单股速览会补充东财 datacenter 已披露财务快照，基金速览会补充公开长期业绩、前端费率、规模和基金经理画像，缺字段保留空值并提示缺口。
...
- 基金速览输出估值/净值、涨跌幅、长期业绩、费率、规模、经理、交易日和前 5 大重仓股报价；基金 `--llm` 深度分析应额外纳入基金画像、持仓结构、重仓股行情和持仓股精选资讯。
...
其他投资框架也会遇到证据包不足：Graham 需要更完整资产质量和下行价值，Munger/Duan Yongping 需要治理、激励和文化证据，Zhang Kun 需要 ROIC、现金流和长期竞争格局，Lynch/O'Neil 需要季度增长、机构需求和量价确认，Wood 需要研发、渗透率和融资风险，Simons 需要样本、因子和交易成本。当前 Evidence Pack 只能覆盖其中一部分；缺少的框架专属证据必须显式列为缺口，不得用市场涨跌、板块热度或专家风格替代。
