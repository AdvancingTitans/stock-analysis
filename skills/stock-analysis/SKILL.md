...
技能只维护日报分析规则、首次投资记忆引导、数据口径约束和输出纪律；行情采集、交易日逻辑、缓存、新闻聚合、基金持仓等实现均来自 CLI 包核心模块。CLI 内部已经将交易日历、投资记忆、日报编排和数据源健康评分拆成独立模块，后续更新这些能力时优先升级 `young-stock-cli>=0.1.16`。需要管理投资记忆时使用 `young profile list/remove-stock/remove-fund/clear/clear-stocks/clear-funds/group create/group add`；卸载可用 `young uninstall`。
