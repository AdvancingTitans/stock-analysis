---
name: stock-analysis
description: 全球股市深度复盘技能。用于 A股、港股、美股、基金的当前行情、盘中/盘后复盘、6 模块证据驱动分析、trading 入口持仓分析、默认 committee 投委会、内置投资专家 lens、单股速览与数据源诊断；执行腾讯/新浪优先、东财独有数据限流、证据质量评分和浏览器接管策略。
metadata:
  version: "4.4.0"
...
- `--market stock --symbol <代码>` 与 `--market fund --symbol <代码>` 是确定性速览入口，不触发 LLM；基金速览会补充公开长期业绩、前端费率、规模和基金经理画像，缺字段保留空值并提示缺口。
...
- 基金速览输出估值/净值、涨跌幅、长期业绩、费率、规模、经理、交易日和前 5 大重仓股报价；不替代基金 `--llm` 深度分析。
